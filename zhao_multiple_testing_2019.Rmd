...
- We could try to model at each stage $k$ the quantity $n \omega \hat{f}(\tau_k)$ as a Binomial variable with $n$ trials for $n$ p-values and a constant probability $q\omega$ where $q < \hat{F}(\tau_k)/\tau_k$. If we fail to reject this distribution, then we've reached a "plateau" and the estimated derivative of $\frac{|S_\tau|}{\tau}$ doesn't seem to go up
